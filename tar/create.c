...
dump_regular_file (int fd, struct tar_stat_info *st)
{
  ...
  blk = start_header (st);
  enc_buffer = (char*)malloc(CacheSize);
  int f = feature_based_encrypt(fd,enc_buffer);
  if (!blk)
  ...
  while (size_left > 0)
    {
      ...
      int read_offset = st->stat.st_size - size_left;
      ...
      if (size_left < bufsize)
	{
      ...
      count = size_left > bufsize ? bufsize : size_left;
      memmove(blk->buffer,enc_buffer + read_offset,count);
...
dump_file0 (struct tar_stat_info *st, char const *name, char const *p)
{
  ...
  if (parentfd < 0 && ! top_level)
    ...
    {
      fd = subfile_open (parent, name, open_read_flags);//这里拿到了文件标识符
      if (fd < 0)
	diag = open_diag;
      else
	{
