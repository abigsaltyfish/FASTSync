...
int main(){
    ...
    FILE* read = fopen("/home/aaa/图片/dataset/new/test.tar","r");
    FILE* write = fopen("/home/aaa/图片/writefile","w");
    unsigned char *fileCache = (char *)malloc(CacheSize);
    int n = feature_based_encrypt(read,fileCache);
    fwrite(fileCache,1,n,write);
